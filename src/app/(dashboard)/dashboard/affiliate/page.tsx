    ...
    <>
      ...
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
        ...
        {params.error === "min_payout" && (
          <div className="p-3 rounded-lg text-sm text-red-400 border border-red-400/20" style={{ background: "rgb(var(--danger-rgb) / 0.08)" }}>
        ...
        {!affiliate ? (
          <div className="card-dark p-6 sm:p-8 max-w-lg mx-auto">
            <div className="text-center mb-6">
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-4 text-2xl" style={{ background: "rgb(var(--accent-rgb) / 0.1)" }}>
                <Link2 size={28} className="text-[var(--accent)]" />
              </div>
              <h2 className="text-xl font-bold text-white">Tham gia chương trình Affiliate</h2>
              <p className="text-gray-400 text-sm mt-2">
                Giới thiệu khoá học cho bạn bè và nhận <span className="text-[var(--accent)] font-semibold">20% hoa hồng</span> mỗi đơn hàng.
              </p>
            </div>

            <div className="space-y-3 mb-6 text-sm text-gray-300">
              <div className="flex items-start gap-3">
                <MousePointerClick size={18} className="text-[var(--accent)] mt-0.5 shrink-0" />
                <span>Nhận link giới thiệu gắn vào bất kỳ trang nào (blog, landing page, khoá học)</span>
              </div>
              <div className="flex items-start gap-3">
                <ShoppingCart size={18} className="text-[var(--accent)] mt-0.5 shrink-0" />
                <span>Cookie tracking 90 ngày — khách mua sau vẫn tính hoa hồng</span>
              </div>
              <div className="flex items-start gap-3">
                <Wallet size={18} className="text-[var(--accent)] mt-0.5 shrink-0" />
          ...
          <>
            ...
            <div className="card-dark p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs px-2 py-0.5 rounded font-semibold" style={{ background: "rgb(var(--accent-rgb) / 0.15)", color: "var(--accent)" }}>
                  {affiliate.ref_code}
                </span>
              </div>
              ...
              <p className="text-xs text-gray-400 mt-2">
                Gắn <code className="text-[var(--accent)]">?ref={affiliate.ref_code}</code> vào bất kỳ URL nào trên buithuha.com
              </p>
              {/* Change ref code */}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <StatCard icon={MousePointerClick} label="Clicks" value={affiliate.total_clicks?.toLocaleString("vi-VN") || "0"} color="var(--info)" />
              <StatCard icon={ShoppingCart} label="Đơn hàng" value={affiliate.total_conversions?.toString() || "0"} color="var(--cat-purple)" />
              <StatCard icon={TrendingUp} label="Tỷ lệ chuyển đổi" value={`${conversionRate}%`} color="var(--warn)" />
              <StatCard icon={Wallet} label="Khả dụng" value={formatCurrency(available)} color="var(--accent)" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="card-dark p-4 text-center">
                ...
                <div className="text-lg font-bold text-[var(--accent)]">{formatCurrency(affiliate.total_paid || 0)}</div>
              </div>
              <div className="card-dark p-4 text-center">
                <div className="text-xs text-gray-400 mb-1">Tỷ lệ hoa hồng</div>
                <div className="text-lg font-bold text-[var(--warn)]">{affiliate.commission_rate}%</div>
              </div>
            </div>
            ...
            {available >= 200000 && affiliate.bank_account && (
              <div className="card-dark p-4 flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-white">Số dư khả dụng: <span className="text-[var(--accent)]">{formatCurrency(available)}</span></div>
              ...
              {conversions.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-gray-400 text-xs border-b border-[var(--surface-2)]">
                    ...
                    <tbody>
                      {conversions.map((c) => (
                        <tr key={c.id} className="border-b border-[var(--surface)]">
                          ...
                          <td className="py-2.5 text-[var(--accent)] font-medium">{formatCurrency(c.commission_amount)}</td>
                          <td className="py-2.5">
                    ...
                    <tbody>
                      {pendingOrders.map((o, i) => (
                        <tr key={i} className="border-b border-[var(--surface)]">
                          ...
                          <td className="py-2.5">
                            <code className="text-xs text-[var(--warn)] bg-[var(--warn)]/10 px-1.5 py-0.5 rounded">DK{o.order_code}</code>
                          </td>
            ...
            <div className="card-dark p-5">
              <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                <Banknote size={16} className="text-[var(--warn)]" />
                Thông tin nhận tiền
              </h3>
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { bg: string; color: string; label: string }> = {
    pending: { bg: "rgb(var(--warn-rgb) / 0.1)", color: "var(--warn)", label: "Chờ duyệt" },
    approved: { bg: "rgb(var(--accent-rgb) / 0.1)", color: "var(--accent)", label: "Đã duyệt" },
    paid: { bg: "rgb(var(--info-rgb) / 0.1)", color: "var(--info)", label: "Đã trả" },
    rejected: { bg: "rgb(var(--danger-rgb) / 0.1)", color: "var(--danger)", label: "Từ chối" },
  };
...
function PayoutStatusBadge({ status }: { status: string }) {
  const config: Record<string, { bg: string; color: string; label: string }> = {
    pending: { bg: "rgb(var(--warn-rgb) / 0.1)", color: "var(--warn)", label: "Đang chờ" },
    processing: { bg: "rgb(var(--info-rgb) / 0.1)", color: "var(--info)", label: "Đang xử lý" },
    completed: { bg: "rgb(var(--success-rgb) / 0.1)", color: "var(--success)", label: "Hoàn tất" },
    failed: { bg: "rgb(var(--danger-rgb) / 0.1)", color: "var(--danger)", label: "Thất bại" },
  };
  const c = config[status] || config.pending;
  return (
    <span className="text-xs px-2 py-0.5 rounded font-medium" style={{ background: c.bg, color: c.color }}>
