...
export default function WaitNode({ data }: { data: any }) {
  ...
  return (
    <div className="px-4 py-3 rounded-xl border border-[var(--warn)] bg-[var(--surface)] min-w-[180px] shadow-lg shadow-amber-900/20">
      <Handle type="target" position={Position.Top} className="!bg-[var(--warn)] !w-3 !h-3 !border-2 !border-[var(--border)]" />
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="text-[10px] text-[var(--warn)] font-bold uppercase">Chờ</div>
          <div className="text-xs text-white font-medium">{label}</div>
        </div>
      </div>
      <Handle type="source" position={Position.Bottom} className="!bg-[var(--warn)] !w-3 !h-3 !border-2 !border-[var(--border)]" />
    </div>
  );
}
