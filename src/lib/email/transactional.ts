...
function getBaseUrl(): string {
  return process.env.NEXT_PUBLIC_APP_URL || "https://buithuha.com";
}

function getSiteDomain(): string {
  try { return new URL(getBaseUrl()).hostname; } catch { return "buithuha.com"; }
}

function getSiteName(): string {
  return process.env.EMAIL_FROM_NAME || "Hà Bùi Academy";
}
...
<body>
  <div class="wrap">
    <div class="logo">
      ...
      <div class="logo-text">Hà Bùi Academy</div>
    </div>
    <div class="card">
      ${content}
    </div>
...
export async function sendWelcomeEmail(to: string, name: string) {
  return sesSendEmail(
    to,
    `Chào mừng ${escapeHtml(name)} đến với Hà Bùi Academy! 🎉`,
    baseTemplate(`
      <h1>Chào mừng bạn, ${escapeHtml(name)}! 🚀</h1>
      <p>Tôi là <span class="highlight">Hà Bùi</span> — và tôi rất vui khi bạn tham gia cộng đồng.</p>
      ...
      <p style="margin:8px 0 0; color:#6b7280; font-size:13px;">— Hà Bùi</p>
    `),
  );
}
...
export async function sendWeeklyNewsletter(
  ...
  return sesSendEmail(
    ...
    baseTemplate(`
      ...
      ${body}
      <div class="divider"></div>
      <p style="margin:0;font-size:13px;color:#6b7280;">— Hà Bùi<br/>
...
export async function sendPasswordResetEmail(
  ...
  return sesSendEmail(
    to,
    `🔑 Đặt lại mật khẩu — Hà Bùi Academy`,
    baseTemplate(`
...
export async function sendAffiliateCommissionEmail(
  ...
  return sesSendEmail(
    to,
    `Bạn vừa nhận hoa hồng ${escapeHtml(formatted)} — Hà Bùi Academy`,
    baseTemplate(`
...
export async function sendVerificationEmail(to: string, name: string, confirmUrl: string) {
  const html = baseTemplate(`
    ...
    <p>Cảm ơn bạn đã đăng ký tài khoản tại <span class="highlight">Hà Bùi Academy</span>.</p>
  ...
  return sesSendEmail(
    to,
    "Xác thực tài khoản Hà Bùi Academy",
    html,
  );
}
